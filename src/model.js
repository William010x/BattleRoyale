...
class Stage {
	constructor(canvas){
		...
		**/
		...
		let numBalls=10, numBoxes=numBalls*10, numOpponents=100;
		// Some balls
...
class Tank extends Actor {
	constructor(stage, position, velocity, colour, radius){
		...
		let command = ["fire", "amunition", "pickup"];
		this.stateVars.concat(command);
	...
	setDirection(dx,dy){
		...
		this.velocity = newDirection.sProd(5);
		/*
		var newVelocity = this.velocity.dotProd(newDirection);
